...
async def test_get_project(client):
    ...
    created = (await client.post("/projects", json={"title": "Get Me"}, headers=headers)).json()
...
async def test_update_project(client):
    ...
    created = (await client.post("/projects", json={"title": "Original"}, headers=headers)).json()
...
async def test_delete_project(client):
    ...
    created = (await client.post("/projects", json={"title": "Delete Me"}, headers=headers)).json()
...
async def test_project_ownership(client):
    ...
    created = (
        await client.post(
            "/projects",
            json={"title": "Owner1 Project"},
            headers={"Authorization": f"Bearer {login1['access_token']}"},
        )
    ).json()

    response = await client.get(
        f"/projects/{created['id']}", headers={"Authorization": f"Bearer {login2['access_token']}"}
    )
