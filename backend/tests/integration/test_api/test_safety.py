...
async def test_create_safety_report(client):
    ...
    proj = (
        await client.post("/projects", json={"title": "Safety Project"}, headers=headers)
    ).json()
    circuit = (
        await client.post(
            "/circuits",
...
async def test_list_safety_reports(client):
    await client.post(
        "/auth/register",
        json={
            "email": "safety_list@example.com",
            "username": "safety_list",
            "password": "SecurePass123!",
        },
    )
    login = (
